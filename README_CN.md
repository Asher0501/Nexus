...
| | LangGraph（中心化） | Nexus（去中心化） |
|---|-------------------|-------------------|
| **执行模型** | 运行时调用你的函数（进程内） | 引擎启动独立进程，等待结果 |
| **谁拥有执行权** | 运行时——它运行你的代码 | 节点——引擎只负责调度和路由 |
| **状态管理** | 集中式 State 对象，外部检查点 | 无共享状态——每个节点只拿到声明的上游数据 |
| **节点身份** | Python 函数，与运行时紧耦合 | 任何能通过 stdin/stdout 输出 JSON 的进程——任意语言、任意运行时 |
| **故障隔离** | 一个节点崩溃 → 整个运行时挂掉 | 一个节点失败 → 引擎路由到错误处理，其他节点不受影响 |
...
| **循环终止** | `interrupt()` + 外部恢复 + 检查点 | `route_policy`（N 轮 / N 秒），无状态——无需检查点 |
| **人工介入** | 在特定节点强制 `interrupt()` | LLM 不确定时自主调用 `ask_human` 工具 |
| **部署方式** | 需要 Python 运行时 | 单个二进制文件（~4MB），零运行时依赖 |

LangGraph 是**图即代码**库——运行时拥有一切。Nexus 是**图即数据**引擎——引擎调度，节点执行。
