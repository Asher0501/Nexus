...
| `command` | `string` | ✅（除 `http`） | 要执行的命令。支持 `{{datarouter.X.content}}`、`{{datarouter.X.route}}`、`{{metadata.run_count}}`、`{{metadata.timed_out}}` 模板插值 |
| `prompt` | `string` | ❌（仅 `llm`） | Prompt 模板。`{{datarouter.X.content}}` 替换为上游输出。省略则自动拼接所有 inputs |
...
- `<alias>` 必须对应 `dataflows` 中某个 `from` 的值
- `<field>` 仅限 `route` 和 `content`（datarouter）或 `run_count` 和 `timed_out`（metadata）


- 替换发生在 spawn 之前
- 不存在的 key 被替换为空字符串
- 普通文本和未识别的模板原样透传，运行时 emit warning 日志
...
```json
{
  "nodes": [
    ...
    },
    {
      ...
      "providers": [{
        "type": "subprocess",
        "command": "cmd.exe /c opencode run --format json --auto --model claude-sonnet-4 -- \"{{datarouter.config.content}}\" 2>nul | python scripts/nexus-wrap.py"
      }],
      "process_timeout_secs": 300
    },
    {
