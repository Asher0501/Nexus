...
7. [LLM Agent 节点](#7-llm-agent-节点type-llm)
   - [7.1 单节点 Claude 调用](#71-单节点-claude-调用)
   - [7.2 单节点 OpenCode 调用](#72-单节点-opencode-调用)
   - [7.3 代码审查循环](#73-代码审查review-fix-retro-循环)
   - [7.4 任意新 CLI](#74-任意新-clinga-codeagent-等)
   - [7.5 逐级上报审批流程](#75-逐级上报审批流程)
8. [边界情况与限制](#8-边界情况与限制)
...
// LLM Agent 节点（通用，支持 claude、opencode 及任意 CLI）
{
  ...
  "prompt": "审查代码: {{inputs.seed}}。回复 JSON: {\"route\":\"approved|rejected\",\"content\":\"...\"}",
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `type` | `"subprocess"` \| `"shell"` \| `"http"` \| `"llm"` | ✅ | 执行方式。`subprocess` 直接 spawn；`shell` 通过 shell 包装；`http` 预留；`llm` 通用 LLM agent 节点 |
| `command` | `string` | ✅（除 `http`） | 要执行的命令。支持 `{{inputs.node_id}}` 和 `{{prompt}}` 模板插值 |
| `prompt` | `string` | ❌（仅 `llm`） | Prompt 模板。`{{inputs.X}}` 替换为上游输出。省略则自动拼接所有 inputs |
| `routes` | `string[]` | ❌（仅 `llm`） | 期望的 route 值列表，注入 prompt 提示 LLM 合法路由。用于分支路由场景 |
...
```json
{
  "nodes": [
    ...
    { "id": "review", "providers": [{
      "type": "subprocess",
      "command": "cmd.exe /c opencode run --format json --auto -- \"{{inputs.config}}\" 2>nul | python scripts/nexus-wrap.py"
...
## 7. LLM Agent 节点（`type: "llm"`）

`type: "llm"` 是通用的 LLM agent 节点类型。用户只需指定 CLI 命令和 prompt，引擎自动处理：跨平台 CLI 发现、子进程管理、实时流式输出写 log、结果解析与路由。
...
3. stdout 逐行转发到引擎 log（不参与路由，仅维测）
4. 进程退出后解析 final output → `{route, content}` → 引擎路由 + DataRouter
  ...
  "nodes": [
    ...
    },
    {
      ...
      "providers": [{
        "type": "llm",
        "command": "claude -p \"{{prompt}}\" --output-format json --verbose",
        "prompt": "审查代码找bug: {{inputs.seed}}。回复JSON: {\"route\":\"approved|rejected\",\"content\":\"审查意见\"}",
        "routes": ["approved", "rejected"]
      }],
      "process_timeout_secs": 120,
...
```json
{
  "nodes": [
    ...
    },
    {
      ...
      "providers": [{
        "type": "subprocess",
        "command": "cmd.exe /c opencode run --format json --auto --model claude-sonnet-4 -- \"{{inputs.config}}\" 2>nul | python scripts/nexus-wrap.py"
      }],
      "process_timeout_secs": 300
    },
    {
