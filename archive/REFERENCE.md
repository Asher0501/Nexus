...
```json
{
  ...
  "edges": [
    {"from": "<上游 ID>", "to": "<下游 ID>", "trigger": "all", "event": "complete", "threshold": 1}
  ],
  "dataflows": [
    {"from": "<上游节点 ID>", "to": "<下游节点 ID>"}
  ]
}
```
...
| `returns` | string[] | ❌ | 可选，声明该节点的返回值列表（用于 exit_reason 分支路由） |
| `edges` | array | ❌ | 调度边数组，定义调度拓扑（from → to），详见下方 SchedulingEdgeDef |
| `dataflows` | array | ❌ | 数据流声明，定义谁的数据传给谁，详见下方 DataFlowDef |
| `max_concurrency` | u64 or null | ❌ | 该节点的最大并发数，null = 继承引擎全局值 |
| `max_retries` | u64 or null | ❌ | 最大重试次数。**注意：重试只对 Timeout 和 SpawnError 生效，exit-code 失败不自动重试**。null = 使用引擎全局默认值 |
...
| `trigger` | "all" \| "any" | ✅ | All: 所有上游到齐后才开始计数。Any: 任一上游事件直接计数 |
| `event` | "complete" \| "failed" \| "timeout" | ✅ | 匹配的事件类型。compelete=exit 0, failed=exit ≠0, timeout=引擎强杀 |
| `exit_reason` | string or null | ❌ | 可选，匹配节点的 stdout 首行 `__nexus_exit_reason:` 声明值 |
| `threshold` | u64 | ❌ | 达到触发所需的事件次数，默认 1 |
...
- `metadata`：执行元信息，包含 `run_count`（当前是第几次执行）和 `timed_out`（上次执行是否超时）

### stdout 输出格式

stdout 无约束格式——节点之间自行协商输出内容。引擎不做路由、不做解析，每行实时上报：
...
引擎将完整 stdout 原样存储到 DataRouter，供下游节点通过 `{{inputs.上游ID}}` 模板插值读取。

**节点可选的流式协议**（引擎照单全收，不做业务路由）：

| 前缀 | 含义 |
|------|------|
| `__nexus_log:` | 中间日志，不计入 output |
| `__nexus_event:` | 结构化的实时事件片段，计入 output |
| `__nexus_log_end` | 关闭前缀检查，后续行全部计入 output |

这些前缀仅用于运行时日志分类，不影响下游节点的数据内容。
...
| 维度 | 取值 | 说明 |
|------|------|------|
| 上游完成 | 节点 exit 0 | 任何上游节点正常退出即视为事件 |
| 组合逻辑 | All / Any | All: 所有上游到齐才触发。Any: 任一上游即触发 |

**边触发逻辑**：

```
下游节点入队条件：所有上游节点均已 Completed
  → 立即入队执行
```

引擎的调度边仅用于拓扑依赖——All 表示等所有上游完成，Any 表示任一上游完成即可。节点之间不通过边做业务路由，不匹配事件类型、不匹配 exit_reason、不设阈值。
...
节点 Timeout / SpawnError
  → retry_count < max_timeout_retries? (默认 3)
    → retry_count++ → 重新执行节点（不触发下游边），边状态重置
...
| clap 4.x | CLI 参数解析 |
| tracing 0.1 | 结构化日志 |
| tracing-subscriber 0.3 | 日志订阅/文件输出 |
