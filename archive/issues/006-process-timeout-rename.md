...
**类型**: 工程实现  
**来源**: Deep Review 问题 1  
**状态**: ✅ **已实现** — ARCHITECTURE.md 全部使用 `process_timeout_secs`，NodeData / NodeParams 均已同步
...
> 引擎只做机械计数和转发，不做业务判断。— 原则 6（见 [`ARCHITECTURE.md`](../architecture/ARCHITECTURE.md) §十）

进程存活超时是"引擎安全网"，属于引擎该做的事。业务计算超时是"节点内部策略"，属于节点自己的事。引擎不应理解或管理业务计算超时。

`ARCHITECTURE.md` 的原则 6 已覆盖此边界——引擎只做机械的事。此 issue 仅将其明确到超时层面。
