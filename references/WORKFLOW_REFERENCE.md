...
for each out_edge of completed_node:
  ...
  5. 如果 event_count >= threshold：
     → triggered = true
     → 如果是 Any 策略：立即入队下游
     → 如果是 All 策略：
         fan_in_pending[to] -= 1
         如果 fan_in_pending[to] == 0：入队下游
         （否则等待其他入边触发）
```

fan-in 场景（A 和 B 完成后触发 C）通过两条独立边实现，每条边各自的计数器独立触发。All 策略下，所有指向同一下游的入边都必须触发后，下游才入队。
...
节点通过在 stdout 中写入 `__nexus_exit_reason: <value>` 行协议来设置退出原因。引擎逐行扫描 stdout，提取 exit_reason 值后与边的 `exit_reason` 字段做精确匹配：
...
节点通过在 stdout 中写入 `__nexus_exit_reason: <value>` 行协议设置退出原因。引擎逐行扫描 stdout，提取 exit_reason 值后与边的 `exit_reason` 字段做精确匹配。
