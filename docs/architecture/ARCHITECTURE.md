...
> Nexus 是一个有向图驱动的插件编排引擎，基于 **局部闭包定理**：全局执行轨迹 = 每个节点局部转移函数的迭代闭包。采用 Syntax/Semantics 分离的设计哲学，将工作流定义、图分析、运行时调度与节点执行解耦。
...
| InvalidEdgeSource | 调度边（edges）中引用的 from 节点不存在 |
| InvalidEdgeTarget | 调度边（edges）中引用的 to 节点不存在 |
| InputSourceNotFound | 数据流（dataflows）中引用的 from 节点不存在 |
| InputTargetNotFound | 数据流（dataflows）中引用的 to 节点不存在 |
| CycleWithoutExit | SCC 中所有节点都没有 threshold > 1 的边，也没有分支出环的边——环无法收敛 |

> **环收敛规则**：Validator 对每个 SCC 检查：
> 1. SCC 中至少有一个节点的某事件类型 threshold > 1（有限迭代），或
> 2. 至少有一个节点有指向 SCC 外部的出边（分支离开）
> 满足其一 → 环收敛。都不满足 → 报 `CycleWithoutExit`。
| InputSourceNotFound | inputs 引用的节点 ID 不存在 |
...
> 引擎只定义 `NodeReady` 一种事件类型。`NodeCompleted` 的处理在 `handle_event()` 节点执行后**同步内联完成**，不经过事件队列。

### 5.2 主事件循环

```
初始化：
  scheduler = Scheduler::new(GraphDef, RuntimeState::new())
  data_router = DataRouter::new(index_map)
  running_count = 0
  max_concurrency = 用户配置 / CPU 数

 从所有入口节点开始：对每个 entry → 发 NodeReady

 事件循环（简化伪代码）：
    收到 NodeReady(node):
      acquire()                    ← 等并发槽位（Semaphore）
      running_count++
      provider = node.providers[0] // 引擎不关心是 subprocess 还是 http
      timeout = node.process_timeout_secs ?? default_node_timeout
      let inputs = data_router.build_input(node);

      // 创建 chunk channel 用于流式输出
      let (chunk_tx, chunk_rx) = mpsc::unbounded_channel();
      spawn: 消费 chunk_rx → 实时 emit 到 tracing

      // NodeShell::from_provider 决定创建什么 executor
      let executor = NodeExecutor::from_provider(provider);
      let outcome = executor.run(inputs, timeout, node_id, chunk_tx).await

      // permit 在函数末尾释放（? 作用域结束自动归还 Semaphore）

      if spawn 失败:
         当作 Failed 处理（无重试）
      else:
        data_router.store_output(node, outcome.output)

        if (outcome.timed_out || outcome.exit_code != 0) && 未超重试次数:
          scheduler.retry_node(node)
          send(NodeReady(node))     // 重试
          running_count--
          continue                  // 跳过下游边处理

        // 确定事件类型后通过 scheduler.handle_event 传播
        event_type = if timed_out { Timeout } else if exit_code == 0 { Complete } else { Failed }
        ready_nodes = scheduler.handle_event(node, event_type, exit_reason)
        for target in ready_nodes: send(NodeReady(target))

      running_count--
      // permit 在此处自动归还（Semaphore 槽位释放）
      // 终止条件：没有节点在跑，也没有节点排队
      // （下一轮循环判断）
```
...
stateDiagram-v2
    ...
    RunningOk --> TerminatedOk : 正常完成（含 Complete 出边）
    RunningOk --> RunningFailed : 超时或 exit ≠ 0
...
```rust
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum NodeStatus { Pending, Running, Terminated }

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum NodeResult { Ok, Failed }
...
节点 Failed 或 Timeout 后，Scheduler 在触发 Failed/Timeout 出边之前，先检查重试计数：
...
| 级别 | 配置项 | 默认值 | 说明 |
|------|--------|--------|------|
| 引擎全局 | `EngineConfig.max_retries` | 3 | 全局默认值 |
...
| 用户配置 | 在 `edges[]` 中声明 | 全局配置 `max_retries` |
| 超出后的行为 | 边 triggered，不再响应 | 触发 Failed/Timeout 出边 |
