...
> **设计补充** · 与 `docs/architecture/ARCHITECTURE.md` 并列，作为 DESIGN_PHILOSOPHY（局部闭包定理）到工程实现之间的映射层。
>
> 审查日期：2026-07-04
> 审查范围：DESIGN_PHILOSOPHY.md（核心命题）、ARCHITECTURE.md（§3.2 Builder、§4.7 索引结构、§5.2 事件循环、§5.4 Scheduler）
> 审查人：Sisyphus
...
- [一、问题](#一问题)
- [二、NodeTransfer 定义](#二nodetransfer-定义)
- [三、Builder 变更：构建 NodeTransfer](#三builder-变更构建-nodetransfer)
- [四、事件循环变更：从遍历所有边到按节点索引](#四事件循环变更从遍历所有边到按节点索引)
- [五、Scheduler 变更：持有 NodeTransfer](#五scheduler-变更持有-nodetransfer)
- [六、发生变更的文件清单](#六发生变更的文件清单)
- [七、不变性 & 正确性证明](#七不变性--正确性证明)
- [八、与局部闭包定理的对应关系](#八与局部闭包定理的对应关系)
- [九、性能影响](#九性能影响)
...
结合 ARCHITECTURE_REVIEW.md #3 的状态分离推演和 All/Any 完备性结论（DESIGN_PHILOSOPHY.md §〇 推论），边的最终形态确定为纯数据结构，无 trait、无多态：
...
**好处**：
- 判定逻辑全部在 `handle_event` 一个函数中，代码结构直接对应数学结构 Fᵥ
...
| 文件 | 变更类型 | 变更内容 |
|------|---------|---------|
| `ARCHITECTURE.md` §3.2 | 文档更新 | BuildResult 新增 `transfers`；`edges` 改为 `Vec<EdgeDef>` |
| `ARCHITECTURE.md` §4.2 | 文档更新 | 移除 `Edge` trait；新增 `EdgeDef`、`EdgeState`、`Strategy` |
| `ARCHITECTURE.md` §4.3/4.4 | 文档更新 | AllEdge/AnyEdge 结构体 + impl 移除 |
| `ARCHITECTURE.md` §4.7 | 文档更新 | 索引结构改为三层：EdgeDef / EdgeState / NodeTransfer |
| `ARCHITECTURE.md` §5.2 | 文档更新 | 事件循环走 handle_event |
| `ARCHITECTURE.md` §5.4 | 文档更新 | Scheduler 新增 transfers、edge_states、handle_event |
| `nexus-engine/src/graph/edge.rs` | 代码变更 | 移除 Edge trait、AllEdge、AnyEdge；新增 EdgeDef、EdgeState、Strategy |
| `nexus-engine/src/graph/scheduler.rs` | 代码变更 | 新增 transfers；handle_event 使用 EdgeDef 字段 |
| `nexus-engine/src/graph/builder.rs` | 代码变更 | 新增 build_transfers()；BuildResult.edges 类型变更 |
| `nexus-engine/src/graph/mod.rs` | 代码变更 | 导出 NodeTransfer、EdgeDef、EdgeState、Strategy |
| `docs/design-extras/NODE_TRANSFER.md` | 文档新增 | 本文 |
