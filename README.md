...
**边 = 四维触发条件。** 事件类型（complete/failed/timeout）× exit_reason 精确匹配 × All/Any 扇入聚合 × threshold 计数。引擎机械计数，不猜测。
...
nexus-cli run workflow.json --max-timeout-retries 5   # 设置超时重试次数
```
...
|------|--------|
| 链式 | A → B → C |
| 扇出/扇入 | A → B, A → C → D（多条独立边，All 策略下所有入边触发后才入队下游） |
| 条件分支 | review 通过 exit_reason 路由：complete/ok → deploy，complete/rejected → fix |
| 自环/阈值 | collector 触发自身 3 次后触发 aggregator |
| 错误处理 | A: complete → B, failed → error_handler（不同事件类型路由不同下游） |
| 并行聚合 | A, B, C 同时执行 → M 等全部完成（与扇入 All 同语义） |
